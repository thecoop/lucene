...
class TermAutomatonScorer extends Scorer {
  ...
  public TermAutomatonScorer(
    ...
    this.docIDQueue =
        PriorityQueue.comparing(subs.length, Comparator.comparingInt(e -> e.posEnum.docID()));
    // Sorts by position so we can visit all scorers on one doc, by position
    this.posQueue = PriorityQueue.comparing(subs.length, Comparator.comparingInt(e -> e.pos));
