...
public class TestCommonTermsQuery extends LuceneTestCase {
  ...
  public void testEqualsHashCode() {
    CommonTermsQuery query =
        new CommonTermsQuery(randomOccur(random()), randomOccur(random()), random().nextFloat());
    int terms = atLeast(2);
...

    {
      ...
      CommonTermsQuery left = new CommonTermsQuery(randomOccur(r), randomOccur(r), r.nextFloat());
      ...
      r = new Random(seed);
      CommonTermsQuery right = new CommonTermsQuery(randomOccur(r), randomOccur(r), r.nextFloat());
    ...
    try {
      ...
      while (iterator.next() != null) {
        if (highFreqQueue.size() < 5) {
          highFreqQueue.add(
              new TermAndFreq(BytesRef.deepCopyOf(iterator.term()), iterator.docFreq()));
          lowFreqQueue.add(
              new TermAndFreq(BytesRef.deepCopyOf(iterator.term()), iterator.docFreq()));
        } else {
  ...
  private static final class ExtendedCommonTermsQuery extends CommonTermsQuery {
    ...
    public ExtendedCommonTermsQuery(
